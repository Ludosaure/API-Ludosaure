...
export class UpdateGameHandler implements ICommandHandler<UpdateGameCommand> {
  ...
  async execute(command: UpdateGameCommand): Promise<Game> {
    const foundGame = await this.gameRepository.findById(command.id);
    if (foundGame == null) {
    ...
    if (command.name != null) {
      const gameWithSameName = await this.gameRepository.findByName(
        command.name,
      );

      const isNameUsedByOtherGame = gameWithSameName.findIndex((game) => {
        return game.id != command.id;
      });

      if (isNameUsedByOtherGame != -1) {
    ...
    if (command.ageMin != null) foundGame.ageMin = command.ageMin;
    if (command.weeklyAmount != null)
      foundGame.weeklyAmount = command.weeklyAmount;

    if (command.isArchived != null) {
      const reservations =
        await this.reservationRepository.findCurrentOrFutureReservationsByGameId(
          foundGame.id,
        );
    ...
    if (command.pictureId != null) {
      const profilePicture = await this.mediaRepository.findById(
        command.pictureId,
      );
